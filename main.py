    ...
    if request.method == "POST":
        ...
        if "file" not in request.files:
            return render_template(
                "message.html",
                message="No icon in upload, maybe the file is corrupted?",
                error=True,
            )

        if request.form["uname"] == "":
            return render_template("message.html", message="No username", error=True)
        ...
        if file.filename == "":
            return render_template(
                "message.html",
                message="No selected icon, maybe you forgot to select a file?",
                error=True,
            )
        ...
        if file and allowed_file(file.filename):
            ...
            if hashed_file in files.keys():
                ...
                users[uname] = (
                    users[uname] + [files[hashed_file]]
                    if uname in users.keys()
                    else [files[hashed_file]]
                )  # Adding to the user's list of icons
            ...
            users[uname] = (
                users[uname] + [filename] if uname in users.keys() else [filename]
            )  # Adding to the user's list of icons
    ...
    if uname in users.keys():
        ...
        if icon == users[uname][0]:
            ...
            # If not, then display an error
                ...
                return render_template(
                    "message.html", message="ERROR NOT USER ICON", error=True
                )

    else:
        return render_template("message.html", message="ERROR NO USER", error=True)
...
def get_best_avatar(uname):
    return (
        send_from_directory(app.config["UPLOAD_FOLDER"], users[uname][0])
        if uname in users.keys()
        else render_template("message.html", message="USER NOT FOUND")
    )
...
def save():
    ...
    return render_template("message.html", message="saved")
...
def load():
    ...
    return render_template("message.html", message=str(users))
